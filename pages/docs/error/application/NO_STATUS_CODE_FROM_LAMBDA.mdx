import Standard from '~/components/layout/standard'
import Note from '~/components/text/note'
import { InlineCode } from '~/components/text/code'

export const meta = {
  title: 'No Status Code from Serverless Function (Lambda)',
  description:
    'The requested Serverless Function failed to respond with a status code.',
  editUrl: 'pages/docs/error/application/NO_STATUS_CODE_FROM_LAMBDA.mdx',
  lastEdited: '2019-09-19T04:07:22.000Z'
}

#### Application Error

The application did not respond with a status code.

<br />

<Note label={false}>
  Code: <InlineCode>NO_STATUS_CODE_FROM_LAMBDA</InlineCode>
</Note>

---

#### Troubleshoot

- Make sure the Serverless Function from the application responds with a status code.
- Check the application logs, which can be found at the host URL under [the `/_logs` path](/docs/v2/advanced/concepts/special-paths#logs-view), for example:
  ```
  https://my-deployment-lwbxweoqjo.now.sh/_logs
  ```

export default ({ children }) => <Standard meta={meta}>{children}</Standard>

export const config = {
  amp: 'hybrid'
}
